...
import type { GameState, Room, GameSettings, PlayerSetupConfig } from '@/types/buta';
import { initializeGame } from '@/lib/buta/gameLogic';
import { useGame } from '@/hooks/buta/useGame';
import {
  isFirebaseEnabled,
  getRoom,
  subscribeToRoom,
  updateGameState,
} from '@/lib/buta/firebase';
import GameBoard from '@/components/buta/GameBoard';
import RoundEndScreen from '@/components/buta/RoundEndScreen';
import GameEndScreen from '@/components/buta/GameEndScreen';
import SoundToggle from '@/components/buta/SoundToggle';
...
export default function OnlineRoomPage() {
    ...
    return (
      <div className="min-h-screen felt-table flex items-center justify-center p-4">
        <div className="glass p-8 max-w-sm w-full text-center">
          ...
          <button
            onClick={() => router.push('/buta/online')}
  ...
  if (!room.gameState || room.status === 'waiting') {
    return (
      <WaitingRoom
        ...
        onLeave={() => router.push('/buta/online')}
      />
    );
  }
  ...
  return (
    <ActiveOnlineGame
      ...
      playerId={playerId}
      roomId={room.id}
      onReturnToLobby={() => router.push('/buta/online')}
    />
  );
}
