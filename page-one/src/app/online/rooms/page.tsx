...
import { isFirebaseEnabled, subscribeToRoomList, joinRoomById } from '@/lib/firebase';

const RANDOM_NAMES = ['さくら', 'ひろし', 'ゆき', 'けんじ', 'あいこ', 'たろう', 'はな', 'しょうた'];
function randomName() {
  ...
  return (
    <div className="min-h-screen felt-table p-4">
      <div className="max-w-md mx-auto">
        ...
        {/* Empty state */}
        {isFirebaseEnabled && !loading && rooms.length === 0 && (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-8 text-center">
            <div className="text-4xl mb-3">🃏</div>
            <p className="text-white/60 text-sm mb-1">現在募集中のルームはありません</p>
            <p className="text-white/40 text-xs mb-5">あなたがルームを作って参加者を待ちましょう！</p>
            <Link
              href="/online"
        ...
        {isFirebaseEnabled && !loading && rooms.length > 0 && (
          <div className="space-y-3">
            {rooms.map((room) => {
              ...
              const maxCount = room.settings.playerCount;
